...
const calculateResources = (reports) => {
  ...
  const distinctLocations = new Set(
    activeReports.map(r => `${Number(r.latitude).toFixed(3)},${Number(r.longitude).toFixed(3)}`)
  );
  const spread = distinctLocations.size;

  // 3. Fire Specific Analysis
  // Count reports that specifically mention "Fire", "Smoke", or "Blast"
  const fireReports = activeReports.filter(r => 
    (r.type && r.type.toLowerCase().includes('fire')) || 
    (r.description && r.description.toLowerCase().includes('fire')) ||
...
app.get('/api/ai-predict', async (req, res) => {
  try {
    ...
    res.json({ 
      active_incidents: result.rows.filter(r => r.status !== 'Resolved').length,
      predictions: predictions // This is now an object: { ambulances, fire_vans, volunteers }
    });
